...
process_execute (const char *file_name) 
{
  ...
  char *ptr;		
  char* fname = strtok_r((char *) file_name, " ", &ptr);
  ...
  else
  {
    ...
    list_push_front(&thread_current()->child_list, &matching_thread->child_elem);
...
process_wait (tid_t child_tid) 
{
 intr_disable ();
  ...
  //free(temp);
  sema_down(&child_thread->waiting);
...
process_exit (void)
{
  ...
  {
  struct list_elem *ele =list_front(&exec_list);
  struct list_elem *next_element;
  while (ele != list_end(&exec_list))
  {
        struct fileFd *ct = list_entry(ele, struct fileFd, f_elem);
        if (strcmp(thread_current()->name, ct->f_name) == 0)
        {
           file_allow_write(ct->file_orig);
           list_remove(ele);
           break;
        }
  ele = list_next(ele);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  if (t->pagedir == NULL) 
    goto done;
  process_activate ();
  char * cp = malloc (strlen(file_name)+1);
  strlcpy(cp, file_name, strlen(file_name)+1);
  char *ptr;
  char* fname = strtok_r((char *) cp, " ", &ptr);
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  /* We arrive here whether the load is successful or not. */
   if (success){
     ...
     memcpy(fl->f_name, (const char *)fname, strlen(fname) + 1);
     list_push_front(&exec_list, &fl->f_elem);
     fl->file_orig = file;
...
setup_stack (void **esp, char* command) 
{
  ...
  len = 1;
  while (strtok_r(NULL, " ", &ptr) != NULL)
  {
    len++;
  }
  free(temp);
  int* pointers = calloc(len,sizeof(int));
  // check tokens
...
while(l >= 0)
{
   *esp = *esp - (strlen(argv[l]) + 1);
   memcpy(*esp, argv[l], strlen(argv[l])+1);
   pointers[l] = *esp;
   l--;
  h++;
}

  //padding for remaining bits
  while((int)*esp%4 != 0)
  {
    char zero = 0;
    *esp = *esp - sizeof(char);
...
{
  *esp= *esp - sizeof(int);
  memcpy(*esp,&pointers[kl],sizeof(int));
  kl--;
}
free(pointers);
char *t = *esp;
